...
def initialization(A, B, p_size):
    ...
    A_padding = np.ones([A_h + p * 2, A_w + p * 2, 4]) * np.nan
    # A_padding = np.ones([A_h + p * 2, A_w + p * 2]) * np.nan
...
def random_search(f, a, dist, A_padding, B, p_size, alpha=0.25):
    x = a[0]
    y = a[1]
    B_h = np.size(B, 0)
    B_w = np.size(B, 1)
