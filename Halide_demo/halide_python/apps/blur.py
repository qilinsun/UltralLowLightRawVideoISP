...
def get_blur(input):
    ...
    assert input.dimensions() == 2 #help 就有了 
    ...
    blur_x[x,y] = (ci[x-2,y] + ci[x-1,y] + 3 * ci[x,y]+ ci[x+1,y]+ci[x+2,y])/7
    blur_y[x,y] = hl.cast(hl.UInt(8), (blur_x[x,y-2]+blur_x[x,y-1] + blur_x[x,y]+blur_x[x,y+1]+blur_x[x,y+2])/5)
...
def main():
    ...
    output_data = np.ones(input_data.shape, dtype=input_data.dtype, order="F")
