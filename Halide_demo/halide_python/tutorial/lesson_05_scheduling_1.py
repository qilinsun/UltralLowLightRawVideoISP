...
if __name__ == "__main__":
    ...
    # # Evaluating in tiles. (先split再reorder就是tile,这么做直观解释就是平均切patch出来)
    # gradient = hl.Func("gradient_tiled")
    # gradient[x, y] = x + y
    # gradient.trace_stores()
    ...
    # #         for y.y_inner in [0, 1]:
    # #             for x.x_inner in [0, 1]:
    # 原本默认顺序是  y_inner  y_outer  x_inner x_outer, 现在要做split再reorder的话 直接
    # # gradient.tile(x, y, x_outer, y_outer, x_inner, y_inner, 2, 2)
    # print("Evaluating gradient in 2x2 tiles")
    # output = gradient.realize([4, 4])
    # gradient.print_loop_nest()

    # # Evaluating in vectors. simply operates using c++ array
    # gradient = hl.Func("gradient_in_vectors")
    # gradient[x, y] = x + y
    # gradient.trace_stores()
    # x_outer, x_inner = hl.Var("x_outer"), hl.Var("x_inner")
    # gradient.split(x, x_outer, x_inner, 4) # 5 10 works..
    # gradient.vectorize(x_inner) # equivalent to: gradient.split(x, x, x_inner, 4)  # gradient.vectorize(x_inner)
    # print("Evaluating gradient with x_inner vectorized ")
    # output = gradient.realize([8, 4])
