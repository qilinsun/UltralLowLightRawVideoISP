...
raw_path = '/media/cuhksz-aci-03/数据/CUHK_SZ/0.1/'
seq_id = "iso6400"
star_id = 0
num_load = 16
p_size = 3
...
#         # y[di::2, dj::2, num] = (denoise_tv_chambolle(np.sqrt(y[di::2, dj::2, num]+(3/8)), weight=0.5, n_iter_max=100))
#         y[di::2, dj::2, num] = (
...
for c, (di, dj) in enumerate(zip([0, 1, 0, 1], [0, 0, 1, 1])):
    ...
    # 'g2w', 'g3w', 'g4w'.
    noise_type = 'g1'
    # noise_var = 0.02 * np.max((y[di::2, dj::2, 0])) ** 2 # Noise variance
...
# for c, (di, dj) in enumerate(zip([0, 1, 0, 1], [0, 0, 1, 1])):
            # noise_variance = estimate_sigma(np.sqrt(ref_basic[di::2, dj::2]+(3/8)))
            # y[di::2, dj::2, 0] = (
            #     denoise_tv_chambolle((y[di::2, dj::2, 0]**2-(3/8)), weight=1, n_iter_max=100))
            # y[di::2, dj::2, 0] = (
            #                 denoise_nl_means((y[di::2, dj::2, 0] ), 8, 5, 0.1))

raw = '/media/cuhksz-aci-03/数据/CUHK_SZ/0.1/iso6400/02527.ARW'
raw = rawpy.imread(raw)

raw.raw_image_visible[740:2020, 2096:4256] = y[:, :, 0]**2-(3/8)
...
cv2.imwrite('/home/cuhksz-aci-03/Documents/UltralLowLightRawVideoISP-main/results/downup_frame_' + str(0) + '.png', (rbg_filimg))
