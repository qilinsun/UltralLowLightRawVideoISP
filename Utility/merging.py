...
def getNoiseParams(tags, blackLevel, whiteLevel, params, options):
    ...
    if you want to read tag:
        import json
        with open("/data/gravitychen/dataset/starlight_denoising/submillilux_videos/submillilux_videos_dng/seq17/0.dng", 'rb') as rawFile:
            tag = exifread.process_file(rawFile)
            jsonstring = json.dumps(tag,sort_keys=True, indent=4,default=str)
...
def mergeChannelHdrplus(referenceChannel, alignedChannelTiles, lambdaS, lambdaR, params, options):
    ...
    lambdaS : 25.24
    lambdaR : 26094
    alignedChannelTiles: [8,63,107,20,20]
    referenceChannelTiles: [63,107,20,20]
    referenceChannel : [640,1080]
    signalLevel : [63,107] 每个 overlap patch 的 root square mean, R^[20,20] -> R^1
    noiseVariance : [63,107,20,20]
    mergedPairsTilesSum: [63,107,20,20]
    mergedChannelTiles: [63,107,20,20]
    params = {'patchSize': 20, 'method': 'DFTWiener', 'noiseCurve': 'exifNoiseProfile'}
    options = {'temporalFactor': 7500.0, 'spatialFactor': 0.1, 'ltmGain': -1, 'gtmContrast': 0.075, '`verbose`': 2}
    '''
    # Initialization
    ...
    if params['method'] == 'DFTWiener':
        ...
        signalLevelRepeat = repeat(signalLevel, 'h w -> h w c d', c=1,d=1)
        noiseVariance = np.multiply(lambdaS, signalLevelRepeat) + lambdaR
        noiseVariance = repeat(noiseVariance, 'h w 1 1-> h w c d', c=params['patchSize'],d=params['patchSize'])
        # noiseVariance = (np.multiply(lambdaS, signalLevelRepeat) + lambdaR).repeat(params['patchSize'], axis=-2).repeat(params['patchSize'], axis=-1)
        if verbose:
...
def mergeHdrplus(referenceImage, alignedTiles, padding, lambdaS, lambdaR, params, options):
    ...
    Args:
        ...
        lambdaS,lambdaR: noise paramter
