...
def wiener(refimg, eps, PSF, K=0.01):

    deblur_img = np.zeros((refimg.shape[0], refimg.shape[1], refimg.shape[2], refimg.shape[3], refimg.shape[4]))
    deblur_img[-1, ...] = refimg[-1, ...]
    for i in range(refimg.shape[0]-1):
        fft_img = np.fft.fft2(refimg[i])

        PSF_fft = np.fft.fft2(PSF[i]) + eps
        PSF_fft_1 = np.conj(PSF_fft) / (np.abs(PSF_fft) ** 2 + K)
        deblurimg = np.fft.fft2(fft_img * PSF_fft_1)
        deblurimg = np.abs(np.fft.fftshift(deblurimg))

        deblur_img[i, ...] = deblurimg
