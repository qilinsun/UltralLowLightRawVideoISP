...
def demo_test_tiff():
    cfg = Config('configs/tiff.yaml')
    pipeline = Pipeline(cfg)


    tiff_path = '/home/cuhksz-aci-02/Desktop/dataset/CRVD/outdoor_raw_noisy/scene1/iso6400/frame0.tiff'
    
    bayer = readTIFF(tiff_path)
    bayer = bayer.reshape((cfg.hardware.raw_height, cfg.hardware.raw_width))

    data, _ = pipeline.execute(bayer)

    output_path = op.join(OUTPUT_DIR, 'test1.png')
