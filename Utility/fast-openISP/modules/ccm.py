...
class CCM(BasicModule):
    def __init__(self, cfg):
        ...
        # ccm = np.array(self.params.ccm, dtype=np.float32).T  # x1024, (4, 3) right-matrix
    ...
    def execute(self, data):
        ...
        # ccm_rgb_image = rgb_image @ self.matrix + self.bias
        # ccm_rgb_image = np.clip(ccm_rgb_image, 0, self.cfg.saturation_values.sdr)

        data['ccm'] = ccm_rgb_image.astype(np.uint16)   
