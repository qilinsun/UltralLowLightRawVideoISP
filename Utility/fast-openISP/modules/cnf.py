...
class CNF(BasicModule):
    def chroma_noise_detection(self, bayer):
    ...
    @staticmethod
    def chroma_noise_correction(array, avg_g, avg_c1, avg_c2, y, gain):
    ...
    def execute(self, data):
        ...
        r, gr, gb, b = split_bayer(bayer, self.cfg.hardware.bayer_pattern)
        avg_r, avg_g, avg_b, is_r_noise, is_b_noise = self.chroma_noise_detection(bayer)

        # y = 0.299 * avg_r + 0.587 * avg_g + 0.114 * avg_b
        y = np.right_shift(306 * avg_r + 601 * avg_g + 117 * avg_b, 10)
        r_cnc = self.chroma_noise_correction(r, avg_g, avg_r, avg_b, y, self.params.r_gain)
        b_cnc = self.chroma_noise_correction(b, avg_g, avg_b, avg_r, y, self.params.b_gain)
        ...
        data['cnf'] = cnf_bayer.astype(np.uint16)
