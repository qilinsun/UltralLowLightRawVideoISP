...
# from 'package'
from .visUtils import addMotionField
from matplotlib import pyplot as plt

def vector_filtering(MVs,threshold=35):
    newMV = np.zeros_like(MVs)
    for idx_mv_frame in range(len(MVs)-1):
        ...
        mask = (diff_mv_length < threshold)
        
        new_current_mv = np.einsum('ijk,ij->ijk',currrent_mv,mask)
        new_next_mv = np.einsum('ijk,ij->ijk',next_mv,mask)
        newMV[idx_mv_frame] = new_current_mv
        newMV[idx_mv_frame+1] = new_next_mv
    return newMV
...
def alignBurst(burstPath, rawPathList, images, refIdx, params, options):
    ...
    if True: #params['mode'] == 'bayer':
		# 'tileSizes': [16, 16, 16, 8],
        tileSize = 2 * params['tuning']['tileSizes'][0] # 2 * 16
    else:
        tileSize = params['tuning']['tileSizes'][0]
    

    # 解决 block 的边界够不够整除的问题，用 pad 
    ...
    # input reference image and following images
...
def alignHdrplus(referenceImage, alternateImages,blockSize):
    ...
    Return
        alignedTiles
            shape: (8, 63, 107, 40, 40)
        motionVectors
            shape: (7, 63, 107, 2) 
    '''
    # For convenience
    currentTime, verbose = time.time(), 2
    ...
    factors =  [1, 2, 4, 4]
    tileSizes =  [blockSize, blockSize, blockSize, blockSize//2]
    ...
    for i, alternateImage in enumerate(alternateImages):
        ...
        # ====================
        #     step 3  align from coarsest to finest level of the pyramid 
        # ====================
        #! 迭代之前帧 和 参考帧 的金字塔
        ...
        # ====================
        #     step 4  get similar tiles according to the motion vector 
        # ====================
    ...
    if True: # writeMotionFields
        saveFolder = "Docs/starlight/align_mismatch/hdrplus_mv"
        if not os.path.isdir(saveFolder):
            os.mkdir(saveFolder)

        unpaddedMotionVectors = vector_filtering(motionVectors)
            # outputName = os.path.join("Docs/starlight/align_mismatch/hdrplus_mv/", f"vector_filter_{i}.png")
        
        unpaddedMotionVectors = motionVectors
        whiteImage = np.zeros([*referenceImage.shape,3],dtype=np.uint8)
        for i, motionVector in enumerate(unpaddedMotionVectors):
            whiteImage_MV = addMotionField(whiteImage, tileSize, unpaddedMotionVectors[i])
            outputName = os.path.join("Docs/starlight/align_mismatch/hdrplus_mv/", f"vector_{i}.png")
            cv2.imwrite(outputName, cv2.cvtColor(whiteImage_MV, cv2.COLOR_RGB2BGR), [cv2.IMWRITE_JPEG_QUALITY, 100])
            # cv2.imwrite(outputName, cv2.cvtColor(whiteImage_MV, cv2.COLOR_RGB2BGR), [cv2.IMWRITE_JPEG_QUALITY, 100])
...
def alignOnALevel(referencePyramidLevel, alternatePyramidLevel, upsamplingFactor=1, tileSize=16, previousTileSize=None, searchRadius=4, distance='L2', subpixel=True, previousAlignments=None):
    ...
    searchAreas = getTiles(np.pad(alternatePyramidLevel, searchRadius, mode='constant', constant_values=2**14 - 1), window=tileSize + 2 * searchRadius,steps=None)
