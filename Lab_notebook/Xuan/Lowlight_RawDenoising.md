...
+ 重新拍摄了灰阶卡的图片，依然分别在低照度和高照度下进行了噪声估计，得到RMS噪声，此值为标准差，即可推出方差。一些资料显示是通过方差和均值进行拟合从而得到加性噪声的方差，但是该软件分析出的噪声只有RMS。
    ...
    + 根据该公式对噪声进行建模：𝒙_𝒑~ 𝝈_𝒔^𝟐 𝓟(𝒚_𝒑/𝝈_𝒔^𝟐) + 𝓟(𝒚_𝒑/𝑵_𝑭𝑷𝑵^𝟐)𝑵_𝑭𝑷𝑵^𝟐 + 𝓝(𝟎,𝝈_𝒓^𝟐)，第一个泊松分布里的参数通过低照度图像估计出来，第二个泊松分布的参数是通过计算同一曝光时间下的30张图片的均值在计算标准差得出，高斯分布的参数通过高照度图像估计出来的。根据该方式建模训练出的模型在测试自己的图像时经过整个pipeline得到的结果是全黑的。
