...
+ 利用卷积响应进行块匹配，[code](https://github.com/qilinsun/UltralLowLightRawVideoISP/tree/main/patchmatch_conv)
  ...
  + 对有所的块进行排序，选出相似度值最高的块，就为当前块的匹配块
  + 然后利用匹配出的块进行图像重建，框架图如下

![](../../Docs/Images/20230526/patch.png)
  
+ 现在普通图像上进行了测试，rgb图，先将图像变为灰度后在执行上述步骤，重建结果如下

![](../../Docs/Images/20230526/重建结果/重建结果对比.png)

从上面结果看，在某些地方还是存在匹配错误的地方，目前还在逐步调试。

------

### 2023.5.20组会

+ 将之前使用的距离匹配方法改成基于特征的图像匹配
+ 利用卷积响应进行块匹配的方法步骤
...
+ Deepmatching和Deepflow测试我们的数据，先测试了由RAW转成RGB图像的光流和映射图，在一些地方存在匹配不上的问题，且编译好的版本只能测试3通道图，deepflow代码不能进行修改，opencv库里有些相应的光流匹配算法，deepflow，dis flow等，用这些方法在进行测试
    ...
    **对比结果**
    ...
    由图中可以看出，静态图使用deepflow结果好于patchmatch。但是在动态图中，想让第5帧图匹配到第1帧，deepflow匹配不回去，patchmatch则能，但是效果不佳
